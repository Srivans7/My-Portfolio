...
export default class ThreejsJourney {
    constructor(_options) {
        ...
        if (!this.$container) {
            // Container not found — disable tour or bail out gracefully
            console.warn('ThreejsJourney: .js-threejs-journey not found. Aborting setup.');
            this.$messages = [];
            this.$yes = null;
            this.$no = null;
            this.step = 0;
            this.maxStep = 0;
            this.shown = false;
            return; // stop further setup if essential
        }
        ...
        this.step = 0;
        this.maxStep = Math.max(0, this.$messages.length - 1);
        this.seenCount = parseInt(window.localStorage.getItem('threejsJourneySeenCount') || '0', 10);
        this.shown = false;
