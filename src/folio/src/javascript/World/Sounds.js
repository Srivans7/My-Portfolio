...
export default class Sounds
{
    ...
    setEngine()
    {
        ...
        // Play engine sound only after a user gesture (autoplay policy)
        // If Web Audio context is already running, play immediately.
        const tryPlayEngine = () => {
            try {
                ...
                // if Howler or ctx is not available for some reason, attempt to play
                try { this.engine.sound.play() } catch (err) {}
            }

            window.removeEventListener('pointerdown', tryPlayEngine)
            window.removeEventListener('touchstart', tryPlayEngine)
            window.removeEventListener('keydown', tryPlayEngine)
        }

        // If audio is allowed already, play; otherwise wait for a user gesture
        if (typeof Howler !== 'undefined' && Howler.ctx && Howler.ctx.state !== 'suspended') {
            this.engine.sound.play()
        } else {
            window.addEventListener('pointerdown', tryPlayEngine)
            window.addEventListener('touchstart', tryPlayEngine)
            window.addEventListener('keydown', tryPlayEngine)
        }
